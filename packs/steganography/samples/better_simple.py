...
from packs.steganography.api.utilities.consts import Bit, zero
from packs.steganography.samples.simple import SimpleEncoder, SimpleDecoder
...
class StegBetterSimpleImageEncrypterDecrypter(StegImageEncrypterDecrypter[str]):
    ...
    def _make_encrypter(self, settings: int) -> StegImageEncrypter:
        return StegBetterSimpleImageEncrypter(
            self._carrier, self._get_suffix(), settings
        )
