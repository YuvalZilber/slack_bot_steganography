...
from packs.steganography.api.common import Steganographer, ImageSteganographer
...
from packs.steganography.api.utilities.functions import FlagIterator, const_iterator
...
class StegEncrypter(Steganographer[CarrierType, ContentType, EncSettingsType], ABC):
    ...
    def __init__(
        self, carrier: CarrierType, suffix: Iterable[Bit], settings: EncSettingsType
    ):
    ...
    def encrypt(self, content: ContentType) -> CarrierType:
        ...
        if msg_length > max_length:
            error_message = (
                f"Can't encode content of length {msg_length} to this carrier. \n"
            ...
            if best_settings is not None:
                error_message += (
                    f"settings that could fit this content: {best_settings}"
                )
            else:
                error_message += (
                    "Even modifying settings won't help you to fit this content."
                )
    ...
    def _best_settings_to_fit_message(
        self, message_size: int
    ) -> Optional[EncSettingsType]:
        """
...
class StegImageEncrypter(
    StegEncrypter[np.ndarray, ContentType, int], ImageSteganographer[ContentType], ABC
):
    def _encrypt(self, content: ContentType) -> np.ndarray:
        ...
        flag = FlagIterator()
        to_encode = itertools.chain(
            zip(settings_bits, const_iterator(1)),
