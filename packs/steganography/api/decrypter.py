...
from packs.steganography.api.utilities.limited_buffer import LimitedBuffer
...
class StegImageDecrypter(
    StegDecrypter[np.ndarray, ContentType, int], ImageSteganographer[ContentType], ABC
):
    def decrypt(self) -> np.ndarray:
        ...
        for pixel in self._flat_image:
            ...
            buffer.write(
                *[(to_decode >> i) & 1 for i in range(bits_per_pixel - 1, 0 - 1, -1)]
            )
