...
from impl.steganography.api.utilities.consts import Bit, one, zero, Byte, Bytes
from impl.steganography.api.utilities.functions import split_bits
...
class SimpleDecoder(BitsDecoder[str]):
    ...
    def decode(self, bits: Iterable[Bit]) -> str:
        bits = list(bits)
        return bytes([self.to_byte(bits[i:i + 8]) for i in range(0, len(bits), 8)]).decode("utf8")
