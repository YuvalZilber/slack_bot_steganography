...
from slacker.backend.commands import allowed_commands
...
class Chat:
    ...
    @property
    def has_error(self) -> bool:  # note: i usually not support this "public property" but it is the only attr to expose
    ...
    def extract_command(self, event) -> str:
        ...
        if cmd not in allowed_commands():
            self.post_error(f"command '{cmd}' not found, try command \\help for more information")
